...
-- Stockage des PEDs shop instance 0
local instance0Peds = {}
...
-- Indique si le shop ouvert est un shop banque (instance 0)
local isBankShop = false
...
function Redzone.Client.Shop.CreateAllPeds()
    ...
    for _, location in ipairs(Config.ShopPeds.Locations) do
        if not location.instance0 then
...
-- =====================================================
-- GESTION DES PEDS INSTANCE 0
-- =====================================================

---Crée les PEDs shop de l'instance 0 (accessibles à tout le monde)
function Redzone.Client.Shop.CreateInstance0Peds()
    Redzone.Client.Shop.DeleteInstance0Peds()

    for _, location in ipairs(Config.ShopPeds.Locations) do
        if location.instance0 then
            local ped = CreateShopPed(location)
            if ped then
                instance0Peds[location.id] = {
                    ped = ped,
                    config = location
                }
            end
    ...
    Redzone.Shared.Debug('[SHOP] PEDs shop instance 0 créés')
end

---Supprime les PEDs shop de l'instance 0
function Redzone.Client.Shop.DeleteInstance0Peds()
    for id, data in pairs(instance0Peds) do
        DeleteShopPed(data.ped)
        instance0Peds[id] = nil
    end
end

---Vérifie si le joueur est proche d'un PED shop instance 0
---@return boolean isNear True si proche
---@return table|nil shopData Les données du PED shop
function Redzone.Client.Shop.IsPlayerNearInstance0Ped()
...
RegisterNUICallback('buyWeapon', function(data, cb)
    ...
    -- Choisir l'événement selon le type de shop (banque ou money)
    if isBankShop then
        TriggerServerEvent('redzone:shop:buyWeaponBank', data.model, quantity)
    else
...
---Démarre le thread d'interaction avec les PEDs shop instance 0
function Redzone.Client.Shop.StartInstance0InteractionThread()
    Redzone.Shared.Debug('[SHOP] Démarrage du thread d\'interaction shop instance 0')
    ...
    CreateThread(function()
        while true do
            ...
            if not Redzone.Client.Teleport.IsInRedzone() then
                ...
                -- Afficher le texte 3D [ARMURERIE] au-dessus des PEDs instance 0 à moins de 15m
                for _, data in pairs(instance0Peds) do
                    if DoesEntityExist(data.ped) then
                        local pedCoords = GetEntityCoords(data.ped)
                        local dist = #(playerCoords - pedCoords)
                        if dist <= 15.0 then
                ...
                if not isShopOpen then
                    local near, shopData = Redzone.Client.Shop.IsPlayerNearInstance0Ped()
...
-- Créer les PEDs instance 0 au chargement du joueur
CreateThread(function()
    Wait(5000)
    Redzone.Client.Shop.CreateInstance0Peds()
    Redzone.Client.Shop.StartInstance0InteractionThread()
end)
