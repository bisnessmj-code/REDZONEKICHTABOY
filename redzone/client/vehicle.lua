...
-- Tracking des véhicules abandonnés pour auto-cleanup
local abandonedVehicles = {} -- [netId] = timestamp quand le véhicule est devenu vide
...
local function DeleteCurrentVehicle()
    ...
    -- Notifier le serveur que le véhicule a été supprimé
    if currentVehicleNetId then
        TriggerServerEvent('redzone:vehicle:unregister', currentVehicleNetId)
    end
    currentVehicle = nil
...
local function SpawnVehicle(vehicleConfig, spawnPoint)
    ...
    if DoesEntityExist(vehicle) then
        ...
        -- Enregistrer le véhicule côté serveur pour le tracking
        TriggerServerEvent('redzone:vehicle:register', netId)
...
---Vérifie si un véhicule a un occupant (conducteur ou passager)
---@param vehicle number Handle du véhicule
---@return boolean hasOccupant True si quelqu'un est dans le véhicule
local function VehicleHasOccupant(vehicle)
    if not DoesEntityExist(vehicle) then return false end
    -- Vérifier conducteur + tous les sièges passagers
...
---Démarre le thread de nettoyage automatique des véhicules abandonnés
function Redzone.Client.Vehicle.StartCleanupThread()
    local cleanupConfig = Config.VehiclePeds.Cleanup
    if not cleanupConfig then return end

    Redzone.Shared.Debug('[VEHICLE] Démarrage du système de nettoyage automatique')

    CreateThread(function()
        while true do
            Wait(cleanupConfig.CheckInterval * 1000)

            -- Seulement dans le redzone
            if Redzone.Client.Teleport.IsInRedzone() then
                ...
                for _, vehicle in ipairs(allVehicles) do
                    if DoesEntityExist(vehicle) then
                        ...
                        if VehicleHasOccupant(vehicle) then
                            -- Véhicule occupé, retirer du tracking abandonné
                            abandonedVehicles[netId] = nil
                        else
                            -- Véhicule vide
                            if not abandonedVehicles[netId] then
                                -- Première détection comme vide, commencer le timer
                                abandonedVehicles[netId] = currentTime
                            else
                                -- Vérifier si le timeout est dépassé
                                local emptyTime = (currentTime - abandonedVehicles[netId]) / 1000
                                if emptyTime >= cleanupConfig.AbandonedTimeout then
                                    -- Ne pas supprimer notre propre véhicule actuel
                                    if vehicle ~= currentVehicle then
                                        -- Supprimer le véhicule abandonné
                                        SetEntityAsMissionEntity(vehicle, true, true)
                ...
                -- Nettoyer les entrées obsolètes du tracking
                for netId, _ in pairs(abandonedVehicles) do
                ...
                if cleanedCount > 0 then
                    Redzone.Shared.Debug('[VEHICLE/CLEANUP] ', cleanedCount, ' véhicule(s) abandonné(s) supprimé(s)')
                end
            end
        end
    end)
...
---Événement: Suppression forcée d'un véhicule par le serveur (quand un joueur quitte/déco)
RegisterNetEvent('redzone:vehicle:forceDelete')
AddEventHandler('redzone:vehicle:forceDelete', function(netId)
    ...
    if vehicle and vehicle ~= 0 and DoesEntityExist(vehicle) then
        ...
        DeleteEntity(vehicle)
        abandonedVehicles[netId] = nil
        Redzone.Shared.Debug('[VEHICLE/CLEANUP] Véhicule forcé supprimé (netId: ', netId, ')')
    end
end)
        ...
        while true do
            ...
            if Redzone.Client.Teleport.IsInRedzone() then
                ...
                if not IsPedInAnyVehicle(playerPed, false) then
                    ...
                    for _, vehicle in ipairs(allVehicles) do
                        if DoesEntityExist(vehicle) then
                            ...
                            if dist < 50.0 then
                                ...
                                -- true = désactiver seulement cette frame (comme le script safezone)
                    ...
                    -- Empêcher le ragdoll au contact véhicule (évite que le joueur vole en l'air)
                    ...
                    -- Restaurer le ragdoll normal
                    if antiCarKillActive then
                ...
                -- Hors du redzone, restaurer le ragdoll normal
                if antiCarKillActive then
                    local playerPed = PlayerPedId()
                    SetPedCanRagdoll(playerPed, true)
                    SetPedCanRagdollFromPlayerImpact(playerPed, true)
...
-- =====================================================
-- PROTECTION ANTI-COLLISION VÉHICULE (vie + armure)
-- =====================================================

-- Valeurs sauvegardées pour la protection anti-collision
local vehicleCollisionProtection = {
    active = false,
    health = 200,
    armor = 0,
}

---Événement: Interception des dégâts en temps réel (même pattern que la safe zone)
AddEventHandler('gameEventTriggered', function(name, args)
    ...
    -- Seulement si c'est nous la victime, en véhicule, dans le redzone
    if victim ~= myPed then return end
    if not vehicleCollisionProtection.active then return end

    -- Vérifier si l'attaquant est un joueur (dégâts de combat légitimes)
    local isPlayerAttack = false
    if DoesEntityExist(attacker) and IsEntityAPed(attacker) and IsPedAPlayer(attacker) then
        isPlayerAttack = true
    end

    if not isPlayerAttack then
        -- Dégâts NON causés par un joueur = collision/mur/objet → restaurer
        SetEntityHealth(myPed, vehicleCollisionProtection.health)
        SetPedArmour(myPed, vehicleCollisionProtection.armor)
        ClearEntityLastDamageEntity(myPed)
    else
        -- Dégâts de combat (tir, etc.) → accepter et mettre à jour les valeurs
        -- Petit délai pour que le jeu applique les dégâts avant de lire les nouvelles valeurs
        SetTimeout(0, function()
            myPed = PlayerPedId()
            vehicleCollisionProtection.health = GetEntityHealth(myPed)
            vehicleCollisionProtection.armor = GetPedArmour(myPed)
        end)
    end
end)

---Thread de backup: protection anti-collision chaque frame (filet de sécurité)
CreateThread(function()
    while true do
        if vehicleCollisionProtection.active then
            local myPed = PlayerPedId()
            local currentHealth = GetEntityHealth(myPed)
            local currentArmor = GetPedArmour(myPed)

            -- Si la vie/armure a augmenté (soin, pickup d'armure), mettre à jour
            if currentHealth > vehicleCollisionProtection.health then
                vehicleCollisionProtection.health = currentHealth
            end
            if currentArmor > vehicleCollisionProtection.armor then
                vehicleCollisionProtection.armor = currentArmor
            end

            -- Si la vie/armure a baissé sans dégât de joueur, restaurer
            if currentHealth < vehicleCollisionProtection.health then
                if not HasEntityBeenDamagedByAnyPed(myPed) then
                    SetEntityHealth(myPed, vehicleCollisionProtection.health)
                    SetPedArmour(myPed, vehicleCollisionProtection.armor)
                end
            end
        ...
        while true do
            ...
            if IsPedInAnyVehicle(playerPed, false) then
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    -- Protection anti-éjection pour LE JOUEUR LOCAL (conducteur OU passager)
                    if inRedzone then
                        sleep = 0
                        SetPedConfigFlag(playerPed, 32, false)       -- Ne pas éjecter à travers le pare-brise
                        SetPedCanBeKnockedOffVehicle(playerPed, 1)   -- Ne pas éjecter du véhicule
                        SetPedCanBeDraggedOut(playerPed, false)      -- Ne pas sortir de force
                    end
                    ...
                    if driver == playerPed then
                        if inRedzone then
                            ...
                            -- Activer la protection anti-collision
                            if not vehicleCollisionProtection.active then
                                vehicleCollisionProtection.health = GetEntityHealth(playerPed)
                                vehicleCollisionProtection.armor = GetPedArmour(playerPed)
                                vehicleCollisionProtection.active = true
                                Redzone.Shared.Debug('[VEHICLE] Protection anti-collision activée')
                            end
                            ...
                            vehicleCollisionProtection.active = false
                        end
                    end
                end
            else
...
-- Timestamp de la dernière sortie de véhicule (pour bloquer la réentrée)
local lastVehicleExitTime = 0

-- Durée en ms pendant laquelle on bloque la réentrée après une sortie
local VEHICLE_EXIT_COOLDOWN = 1000
                    ...
                    if IsDisabledControlJustPressed(0, 75) then
                        ...
                        if DoesEntityExist(vehicle) then
                            ...
                            if playerSeat == -1 or playerSeat == 1 then
                                ...
                                offset = GetOffsetFromEntityInWorldCoords(vehicle, -2.5, 0.0, 0.0)
                            else
                            ...
                            -- Marquer le timestamp de sortie pour bloquer la réentrée
                            lastVehicleExitTime = GetGameTimer()

                            Redzone.Shared.Debug('[VEHICLE] Descente instantanée effectuée')
                        end
                    end
                else
                    -- Le joueur est à pied : bloquer la réentrée automatique après une sortie
                    if lastVehicleExitTime > 0 and (GetGameTimer() - lastVehicleExitTime) < VEHICLE_EXIT_COOLDOWN then
                        sleep = 0

                        -- Bloquer toutes les touches d'entrée véhicule
                        DisableControlAction(0, 23, true)  -- INPUT_ENTER (F à pied)
                        DisableControlAction(0, 75, true)  -- INPUT_VEH_EXIT

                        -- Annuler toute tâche d'entrée dans un véhicule
                        playerPed = PlayerPedId()
                        if GetIsTaskActive(playerPed, 160) then -- TASK_ENTER_VEHICLE
                            ClearPedTasksImmediately(playerPed)
                        end
                    else
                        lastVehicleExitTime = 0
                    end
                end
            end
...
CreateThread(function()
    ...
    Redzone.Client.Vehicle.StartCleanupThread()
end)
