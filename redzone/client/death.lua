...
function Redzone.Client.Death.OnPlayerDeath(forceKillerServerId)
    ...
    print('[REDZONE/DEBUG CLIENT] Joueur mort - isDead = true')
    ...
    print('[REDZONE/DEBUG CLIENT] Vie après resurrection: ' .. GetEntityHealth(playerPed) .. ' | Invincible: OUI | Animation: lancée')
    print('[REDZONE/DEBUG CLIENT] TriggerServerEvent redzone:death:playerDied va être appelé')
...
-- =====================================================
-- BLOCAGE INVENTAIRE QUAND MORT
-- =====================================================

-- Intercepter la commande 'inventory' de qs-inventory (liée à TAB)
-- Si le joueur est mort, on ferme immédiatement l'inventaire
RegisterNetEvent('qs-inventory:client:openInventory')
AddEventHandler('qs-inventory:client:openInventory', function()
    if isDead and IsInRedzone() then
        print('[REDZONE/DEBUG] Inventaire bloqué - joueur mort')
...
-- Thread de sécurité : si l'inventaire s'ouvre quand le joueur est mort, le fermer
-- et remettre l'animation de mort
CreateThread(function()
    while true do
        if isDead and IsInRedzone() then
            -- Vérifier si le joueur s'est relevé (animation interrompue par l'inventaire)
            -- Seulement si le joueur n'est PAS en train d'être porté
            if not isBeingCarried then
                local playerPed = PlayerPedId()
                if not IsEntityPlayingAnim(playerPed, 'timetable@tracy@sleep@', 'idle_b', 3) then
                    print('[REDZONE/DEBUG] Animation mort interrompue - remise en place')
                    -- Fermer l'inventaire
                    TriggerEvent('qs-inventory:client:closeInventory')
                    TriggerEvent('inventory:client:forceCloseInventory')
                    TriggerEvent('inventory:client:closeinv')

                    -- Remettre l'animation de mort
                    local animDict = 'timetable@tracy@sleep@'
                    RequestAnimDict(animDict)
                    local t = 50
                    while not HasAnimDictLoaded(animDict) and t > 0 do Wait(10) t = t - 1 end
