...
AddEventHandler('redzone:death:playerDied', function(killerServerId)
    ...
    -- Bloquer l'inventaire du joueur mort (côté serveur pour que qs-inventory le respecte)
    Player(source).state:set('inv_busy', true, true)

    print('[REDZONE/DEBUG] inv_busy = TRUE pour joueur ' .. source .. ' (mort)')
...
AddEventHandler('redzone:death:playerRevived', function()
    ...
    -- Débloquer l'inventaire du joueur
    Player(source).state:set('inv_busy', false, true)

    print('[REDZONE/DEBUG] inv_busy = FALSE pour joueur ' .. source .. ' (réanimé/respawn)')
...
AddEventHandler('playerDropped', function(reason)
    ...
    if deadPlayers[source] then
        deadPlayers[source] = nil
        -- Débloquer l'inventaire au cas où
        pcall(function() Player(source).state:set('inv_busy', false, true) end)
    end
