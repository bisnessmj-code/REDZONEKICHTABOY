...
-- =====================================================
-- ÉVÉNEMENT: Achat via compte en banque (Instance 0)
-- =====================================================

---Événement: Achat d'un produit payé avec le compte en banque
---@param productModel string Le modèle du produit
---@param quantity number|nil La quantité (pour les munitions uniquement)
RegisterNetEvent('redzone:shop:buyWeaponBank')
AddEventHandler('redzone:shop:buyWeaponBank', function(productModel, quantity)
    local source = source
    ...
    -- Vérifier le solde du compte en banque
    local bankMoney = xPlayer.getAccount('bank').money

    if bankMoney < finalPrice then
        local manque = finalPrice - bankMoney
        Redzone.Server.Utils.NotifyError(source, 'Fonds insuffisants en banque! Il vous manque $' .. manque)
        Redzone.Shared.Debug('[SHOP] Fonds banque insuffisants pour joueur ', source, ': besoin ', finalPrice, ', a ', bankMoney)
        return
    end

    -- Retirer l'argent du compte en banque
    ...
    Redzone.Server.Utils.NotifySuccess(source, productData.name .. quantityText .. ' acheté pour $' .. finalPrice .. discountText .. ' (Banque)')

    Redzone.Shared.Debug('[SHOP] Achat BANQUE par joueur ', source, ': ', productModel, quantityText, ' pour $', finalPrice)
end)
