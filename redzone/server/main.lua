...
-- Véhicules des joueurs (source -> netId)
local playerVehicles = {}

-- Forward declaration (utilisée avant sa définition)
local DeletePlayerVehicle
...
AddEventHandler('redzone:playerLeft', function()
    ...
    if Config.VehiclePeds.Cleanup and Config.VehiclePeds.Cleanup.DeleteOnLeave then
...
AddEventHandler('playerDropped', function(reason)
    ...
    -- Supprimer le véhicule du joueur qui se déconnecte
    if Config.VehiclePeds.Cleanup and Config.VehiclePeds.Cleanup.DeleteOnDisconnect then
        DeletePlayerVehicle(source)
    end
...
local function KickPlayerFromRedzone(targetId, kickedBy)
    ...
    -- Supprimer le véhicule du joueur kické
    DeletePlayerVehicle(targetId)
...
-- =====================================================
-- GESTION DES VÉHICULES (TRACKING + CLEANUP)
-- =====================================================

---Supprime le véhicule d'un joueur en notifiant tous les clients dans le redzone
---@param source number L'ID du joueur dont le véhicule doit être supprimé
DeletePlayerVehicle = function(source)
    local netId = playerVehicles[source]
    if not netId then return end

    -- Notifier tous les joueurs dans le redzone de supprimer ce véhicule
    for playerId, _ in pairs(playersInRedzone) do
        TriggerClientEvent('redzone:vehicle:forceDelete', playerId, netId)
    end

    playerVehicles[source] = nil
    Redzone.Shared.Debug('[SERVER/VEHICLE] Véhicule supprimé pour joueur ', source, ' (netId: ', netId, ')')
end

---Événement: Un joueur enregistre son véhicule spawné
RegisterNetEvent('redzone:vehicle:register')
AddEventHandler('redzone:vehicle:register', function(netId)
    local source = source
    if not Redzone.Server.Utils.IsPlayerConnected(source) then return end

    -- Supprimer l'ancien véhicule s'il en avait un
    if playerVehicles[source] then
        DeletePlayerVehicle(source)
    end

    playerVehicles[source] = netId
    Redzone.Shared.Debug('[SERVER/VEHICLE] Véhicule enregistré - Joueur: ', source, ', NetId: ', netId)
end)

---Événement: Un joueur supprime son véhicule
RegisterNetEvent('redzone:vehicle:unregister')
AddEventHandler('redzone:vehicle:unregister', function(netId)
    local source = source
    if playerVehicles[source] == netId then
        playerVehicles[source] = nil
        Redzone.Shared.Debug('[SERVER/VEHICLE] Véhicule désenregistré - Joueur: ', source)
    end
end)
